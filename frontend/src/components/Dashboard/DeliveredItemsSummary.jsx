                            ...
                            <div className="col-8">
                                {deliveredItemsData.length === 0 ? (
                                    ...
                                    deliveredItemsData.map((value, index) => (
                                        <h2 className="d-flex align-items-center mb-0" key={index}>{numeral(value.deliveredItems).format('0,0')}</h2>
                                    ))
                                )}
                            </div>
                        </div>
                        <div className="progress mt-1" style={{ height: '8px' }}>
                            <div className="progress-bar l-bg-cyan" role="progressbar" aria-valuenow={progressValue} aria-valuemin={minProgressValue} aria-valuemax={maxProgressValue} style={{ width: `${progressValue}%` }} aria-label="delivered-items-progress-bar"/>
                        </div>
                    </div>
                </div>
            </div>
